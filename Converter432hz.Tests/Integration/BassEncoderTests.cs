...
public class BassEncoderTests
{
    public BassEncoderTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    public IBassEncoder Model => _model ??= SetupModel();
    private IBassEncoder _model;
    private IBassEncoder SetupModel()
    {
        BassDevice.Instance.Init(0);
        return new BassEncoder(FileSystem);
    }
    ...
    {
        var file = new ProcessingItem("Converter432hz.dll", "Converter432hz.dll") { Destination = "_out" };
