    ...
    public class MediaEncoderBusiness {
        ...
        public async Task PreparePreviewFile(MediaEncoderSettings settings, bool overwrite, bool calcAutoCrop) {
            ...
            if (!AviFileReady && settings.ConvertToAvi)
                AviFileReady = await Task.Run(() => FFmpeg.MediaEncoder.ConvertToAvi(settings.FilePath, PathManager.PreviewSourceFile, true, new ProcessStartOptions(FFmpegDisplayMode.Interface, "Converting to AVI"))) == CompletionStatus.Success;
            ...
            if (calcAutoCrop) {
                if (settings.CropLeft == 0 && settings.CropTop == 0 && settings.CropRight == 0 && settings.CropBottom == 0) {
                    Rect AutoCrop = await Task.Run(() => EncoderBusiness.GetAutoCropRect(settings.FilePath, settings.SourceWidth ?? 0, settings.SourceHeight ?? 0, null));
        ...
        public async Task RunGpuTest() {
            await Task.Run(() => AvisynthEnv.GpuSupport);
        }
